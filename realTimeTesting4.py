...
def checkhandIn3(hand_check):
    global handin
    #handin = False
    graybox = cv2.cvtColor(hand_check, cv2.COLOR_BGR2GRAY)
    ...
    for cnt in contours:
        ...
        if areaCnt < 9000 and areaCnt > 2000:
            print("hand in")
            print(areaCnt)
            handin = True
            break
...
def main():
    ...
    json_file = open('3dcnnresult/84c/3dcnnmodel.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)

    # load weights into new model
    loaded_model.load_weights("3dcnnresult/84c/3dcnnmodel.hdf5")
    ...
    closeclipdist_in_meters = 0.1
    closeclipdist = closeclipdist_in_meters / depth_scale
    hand_clipping_distance = clipping_distance
    ...
    try:
        while True:
            ...
            if (backgroundRemove == True):
                ...
                bg_removed = np.where((depth_image_3d > clipping_distance) | (depth_image_3d < closeclipdist) | (depth_image_3d <= 0), grey_color,
                                      color_image)
            ...
            if handin == True:
                cv2.circle(draw_image, (110, 350), 20, (0, 255, 0), 10)
            else:
                cv2.circle(draw_image, (110, 350), 20, (0, 0, 255), 10)

            cv2.rectangle(draw_image, (10, 300), (200, 480), (0, 255, 255), 2)
            ...
            if delayBol == False and gestStart == True:
                ...
                if depthFrame == maxFrames:
                    ...
                    #if updatedWeight:
                        #weightI = manipWeight(weightDense22I, ignoreG, 1)
                    ...
                    #if updatedWeight2:
                        #maxClass = 10
                        #weightA = manipWeight(weightDense22A, availG, 1)
