...
def checkhandIn2(boxCounter, deptRef, midx, midy, w, h, depth_imageS, boxColor, draw_image,is_in=False):
    ...
    if boxCounter == True:
        ...
        for cnt in contours:
            ...
            if areaCnt > 500:
                ...
                handx = xb
                handin = True
                im = cv2.drawContours(graybox, contours, idx, (0,255,0), 3)
                cv2.imshow("title", im)    
                cv2.waitKey(10)
                break
        cv2.rectangle(draw_image, (boxXup.__int__(), boxYup.__int__()), (boxXdn.__int__(), boxYdn.__int__()),
...
def main():
    ...
    nameGest = ['call', 'scroll up', 'scroll down', 'right', 'back', "like", 'play/pause',
                'close', 'click', 'down']
    ...
    try:
        while True:
            ...
            gray = color_image
            grays = cv2.cvtColor(color_image, cv2.COLOR_BGR2GRAY)
            if gestCatch == False:
                faces = face_cascade.detectMultiScale(grays, 1.1, 2)
